...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['lawyer', 'researcher'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<LawfirmInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/lawfirms` : null),
    () => getLawfirms({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Lawfirm',
      path: isTenantUser ? `/lawfirms/view/${data?.[0]?.id}` : '/lawfirms',
      entity: 'lawfirm',
