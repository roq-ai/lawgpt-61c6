...
import { getSimilarCaseById } from 'apiSdk/similar-cases';
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Similar Case Detail View
          </Text>
          {hasAccess('similar_case', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/similar-cases/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              {hasAccess('renamedcase', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/renamedcases/view/${data?.Renamedcase?.id}`}>
                      {data?.Renamedcase?.details}
                    </Link>
                  </Text>
                </Flex>
              )}
