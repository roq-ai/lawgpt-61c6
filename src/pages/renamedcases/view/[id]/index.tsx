...
import { getRenamedcaseById } from 'apiSdk/renamedcases';
...
import { deleteSimilarCaseById } from 'apiSdk/similar-cases';

function RenamedcaseViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RenamedcaseInterface>(
    ...
    () =>
      getRenamedcaseById(id, {
        relations: ['lawfirm', 'similar_case'],
      }),
  );

  const similar_caseHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Renamedcase Detail View
          </Text>
          {hasAccess('renamedcase', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/renamedcases/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              {hasAccess('lawfirm', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/lawfirms/view/${data?.lawfirm?.id}`}>
                      {data?.lawfirm?.name}
                    </Link>
                  </Text>
                </Flex>
              ...
              {hasAccess('similar_case', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/similar-cases/create?case_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.similar_case?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/similar-cases/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.link}</Td>
                            ...
                            <Td>
                              {hasAccess('similar_case', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/similar-cases/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('similar_case', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    similar_caseHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
