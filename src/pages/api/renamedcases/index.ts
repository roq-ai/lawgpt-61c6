import type { NextApiRequest, NextApiResponse } from 'next';
import { roqClient } from 'server/roq';
import { prisma } from 'server/db';
import { authorizationValidationMiddleware, errorHandlerMiddleware } from 'server/middlewares';
import { renamedcaseValidationSchema } from 'validationSchema/renamedcases';
import { convertQueryToPrismaUtil } from 'server/utils';
import { getServerSession } from '@roq/nextjs';

async function handler(req: NextApiRequest, res: NextApiResponse) {
  const { roqUserId, user } = await getServerSession(req);
  switch (req.method) {
    case 'GET':
      return getRenamedcases();
    case 'POST':
      return createRenamedcase();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getRenamedcases() {
    const data = await prisma.renamedcase
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'renamedcase'));
    return res.status(200).json(data);
  }

  async function createRenamedcase() {
    await renamedcaseValidationSchema.validate(req.body);
    const body = { ...req.body };
    if (body?.similar_case?.length > 0) {
      const create_similar_case = body.similar_case;
      body.similar_case = {
        create: create_similar_case,
      };
    } else {
      delete body.similar_case;
    }
    const data = await prisma.renamedcase.create({
      data: body,
    });
    return res.status(200).json(data);
  }
}

export default function apiHandler(req: NextApiRequest, res: NextApiResponse) {
  return errorHandlerMiddleware(authorizationValidationMiddleware(handler))(req, res);
}
